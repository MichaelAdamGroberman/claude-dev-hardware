...
// Character evolution milestones (lifetime tokens). All five stages are
// REACHABLE through normal leveling so the mascot visibly gains its upgrades —
// chest/neck/mouth → antenna LED → sunglasses → HUD shading/particles/LCD →
// the full Ascended loadout (gear + costume + DJ) — as lifetime usage climbs.
// Stages 1–4 arrive every ~5 levels (250K) and Stage 5 (Ascended) lands at
// 1.25M, just past Stage 4, so the endgame form arrives organically rather than
// at an unreachable figure. Kept in sync with the daemon's _EVO_MILESTONES.
...
struct Settings {
  ...
  bool hud;
  bool dj;       // Stage-5 DJ flourish enable (auto end-game mode)
  uint8_t clockRot;  // 0=auto 1=portrait 2=landscape
  bool cycle;    // showcase: auto-rotate the character through every evolution
                 // form (skin) and persona mood so all upgrade designs are seen
};

static Settings _settings = { true, true, false, true, false, true, false, 0, false };
...
inline void settingsLoad() {
  ...
  _settings.dj       = _prefs.getBool("s_djm", false);  // default OFF (renamed key forces the new default past any stored s_dj)
  _settings.clockRot = _prefs.getUChar("s_crot", 0);
  if (_settings.clockRot > 2) _settings.clockRot = 0;
  _settings.cycle = _prefs.getBool("s_cyc", false);  // showcase rotate, default off
  _prefs.end();
}
...
inline void settingsSave() {
  ...
  _prefs.putBool("s_djm", _settings.dj);
  _prefs.putUChar("s_crot", _settings.clockRot);
  _prefs.putBool("s_cyc", _settings.cycle);
  _prefs.end();
}
