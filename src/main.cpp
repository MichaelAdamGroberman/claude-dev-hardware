...
#include "data.h"
#include "buddy.h"
#include "buddy_common.h"   // BUDDY_CANVAS_W / BUDDY_Y_BASE / BUDDY_BG (EQ zone clear)
#include "mic.h"
#include "net_wifi.h"
#include "net_wg.h"
#include "net_tcp.h"
...
// gr0m tilt-rides (#tilt-skate/#tilt-surf/#tilt-hover) are now an automatic
// Stage-5 idle flourish handled entirely inside gr0m.cpp's drawRide() — no
// manual cycle control is exposed here anymore.
...
const uint32_t SCREEN_OFF_MS = 15000;   // aggressive saver: ~15s idle → screen off (battery only; USB stays on)
// Showcase rotation period (Settings ▸ cycle). Every interval the character
// advances to the next finished outfit (outfit = step % gr0mOutfitCount()).
const uint32_t SHOWCASE_MS = 8000;
...
// Aggressive battery saver: on battery, cap the backlight at LOWPWR_BATT_MAX
// even when the user set it higher — the LCD backlight is the single biggest
// drain. On USB (charging) honor the full setting so the desk clock stays
// bright. This runs only on wake / brightness change / dim-restore (never
// per-frame), so the live VBus read here is cheap and keeps the cap in sync
// with plug/unplug at the next interaction.
static const uint8_t LOWPWR_BATT_MAX = 2;   // 0..4 → caps ScreenBreath at 60
static void applyBrightness() {
  uint8_t lvl = brightLevel;
  if (M5.Axp.GetVBusVoltage() <= 4.0f && lvl > LOWPWR_BATT_MAX) lvl = LOWPWR_BATT_MAX;
  M5.Axp.ScreenBreath(20 + lvl * 20);
}
...
// loop(), unlocked at Stage 5), so there's no "dj" row here. The "world"
// (worldUp) and "ride" rows were also removed: world-up was niche/confusing
// (and caused an approval-screen bug), and the tilt-rides are now an automatic
// Stage-5 idle flourish (see drawRide / the rides flourish in loop()) rather
// than a user toggle — both are automatic leveling upgrades, not settings.
...
// 2 led, 3 mic, 4 transcript(hud), 5 dj, 6 cycle, 7 clock rot, 8 ascii pet,
// 9 reset, 10 back. "cycle" is a showcase toggle: when on, the character
// auto-rotates through every evolution form (skin) AND persona mood, so all the
// upgrade designs are seen on a timer instead of waiting for the token-milestone
// / time-of-day conditions that normally drive them.
const char* settingsItems[] = { "bright", "sound", "led", "mic", "hud", "dj", "cycle", "clock", "pet", "reset", "back" };
...
static void applySetting(uint8_t idx) {
  ...
  switch (idx) {
    ...
    case 5: s.dj = !s.dj; break;     // Stage-5 DJ flourish enable
    case 6:  // cycle — showcase: auto-rotate every skin + mood
      s.cycle = !s.cycle;
      buddyInvalidate();   // repaint immediately; rotation isn't a state/species change
      break;
...
// Header strip — the home-screen top bar. "gr0m" is the screen TITLE, so it
// takes the HEADER role (size 2 white) exactly like every other screen header.
// The right cluster (status dot + IDLE/BUSY state label + battery glyph) is the
// strip's compact status tag — size-1, the same SUB caption role the page
// counters and the adapter PROBE tag use on their header strips — kept on the
// right so it doesn't collide with the wider size-2 wordmark.
...
static void drawHeaderStrip(const char* stateLbl, uint16_t stateCol, uint8_t pct) {
  ...
  spr.setTextSize(FS_HEADER);                 // HEADER role: size 2 white title
  spr.setTextColor(DS_WHITE, DS_BLACK);
  spr.setCursor(6, 3);
  spr.print("gr0m");
  // Right cluster, laid out right-to-left: battery (18+3 wide) | state | dot.
  // All size-1 (the strip's status caption), vertically centred in the 22 px.
  int bx = W - 6 - 21;
  drawBatteryGlyph(bx, 6, pct);
  ...
  spr.setCursor(sx, 8);
  spr.print(stateLbl);
  drawStatusDot(sx - 8, 10, stateCol, 3, true);
  spr.drawFastHLine(0, H_STRIP - 1, W, DS_HDRBORD);
}
...
// ── Canonical screen header strip ────────────────────────────────────────
// THE single source of truth for a full-screen header. Every full screen's
// top strip routes through this so the HEADER role is byte-identical
// everywhere: title is FS_HEADER (size 2) white, the right-aligned counter
// (page/selection "n/total") is FS_SUB (size 1) dim, hairline divider under.
// `bg` is the strip fill — DS_HEADER/DS_DARK for the neutral screens, or a
// semantic band (blue passkey / amber adapter / red destructive) for screens
// that intentionally colour-code their strip. The title TYPOGRAPHY stays
// size-2-white regardless of band colour, so a header reads the same on every
// screen. `counter` may be null (no counter shown). Returns the y just below
// the strip (strip occupies rows [y0, y0+H_HDR]; divider at y0+H_HDR).
const int H_HDR = 20;
static int drawScreenHeader(const char* title, const char* counter,
                            uint16_t bg, int y0 = 0) {
  spr.fillRect(0, y0, W, H_HDR, bg);
  spr.drawFastHLine(0, y0 + H_HDR, W, DS_HDRBORD);
  spr.setTextSize(FS_HEADER);                 // HEADER = size 2, white-on-dark
  spr.setTextColor(DS_WHITE, bg);
  spr.setCursor(6, y0 + 3);
  spr.print(title);
  if (counter && counter[0]) {                // page/selection counter = SUB dim
    spr.setTextSize(FS_SUB);
    spr.setTextColor(DS_DIM, bg);
    spr.setCursor(W - 6 - (int)strlen(counter) * 6, y0 + 7);
    spr.print(counter);
  }
  return y0 + H_HDR + 1;
}
...
static void drawListMenu(const char* title, const char* const* items, int n,
  ...
  const int HEADER_H = H_HDR, FOOTER_H = 14;
  // Header strip — routed through the canonical helper so the title (size 2
  // white) and the n/total counter (size 1 dim) match every other screen.
  char cb[12]; snprintf(cb, sizeof(cb), "%d/%d", sel + 1, n);
  drawScreenHeader(title, cb, DS_HEADER);
...
// clock-rot and ascii-pet show their cycler value. Reset (idx 9) carries the
// destructive "!" marker; back (idx 10) has no meta.
static void settingsMeta(int i, char* out, size_t osz, uint16_t* col, bool* danger) {
  ...
  } else if (i >= 1 && i <= 6) {             // sound/led/mic/hud/dj/cycle on-off
    bool on = (i == 1) ? s.sound : (i == 2) ? s.led : (i == 3) ? s.mic
            : (i == 4) ? s.hud  : (i == 5) ? s.dj  : s.cycle;
    snprintf(out, osz, "%s", on ? "on" : "off");
    *col = on ? DS_GREEN : DS_DIM;
  } else if (i == 7) {                       // clock rotation
    static const char* const RN[] = { "auto", "port", "land" };
    snprintf(out, osz, "%s", RN[s.clockRot]); *col = DS_ORANGE;
  } else if (i == 8) {                       // ascii/gif pet
    uint8_t total = buddySpeciesCount() + (gifAvailable ? 1 : 0);
    uint8_t pos   = buddyMode ? buddySpeciesIdx() + 1 : total;
    snprintf(out, osz, "%u/%u", pos, total); *col = DS_ORANGE;
  } else if (i == 9) {                       // reset (destructive)
...
// (over the list) once a destructive row is armed. Red "DESTRUCTIVE" header +
// "n/2" tap counter + the action sentence + the action footer. Per the
// artboard the footer is A RESET (red, left) / B CANCEL (green, right): BtnA
// is the FINAL TAP that executes, BtnB cancels (see loop()).
static void drawResetConfirm(const char* action, const char* l1, const char* l2,
                             const char* l3) {
  spr.fillSprite(DS_CFMBG);
  // Red destructive header bar — the alarm-red exception (like the approval
  // bar): its COLOUR treatment (black-on-red) is intentional, but its SIZES
  // follow the type system. Title is the canonical size-2 HEADER role reading
  // "DESTRUCTIVE" (the artboard's lead word). At the size-2 floor "DESTRUCTIVE"
  // already fills the full 135-px monospace row, so the "1/2" tap counter (the
  // size-1 SUB caption every other screen uses) is carried just under the
  // strip instead of overlapping the title — same information, no collision.
  spr.fillRect(0, 0, W, H_HDR, DS_CFMHDR);
  spr.drawFastHLine(0, H_HDR, W, DS_HDRBORD);
  spr.setTextSize(FS_HEADER);
  spr.setTextColor(DS_BLACK, DS_CFMHDR);
  spr.setCursor(2, 3);  spr.print("DESTRUCTIVE");

  // Body — tap counter (SUB), action label (dim red), big white sentence.
  spr.setTextSize(FS_SUB);
  spr.setTextColor(DS_DIM, DS_CFMBG);
  spr.setCursor(W - 6 - 3 * 6, H_HDR + 6);  spr.print("1/2");
  int y = 56;
  spr.setTextSize(1);
  ...
  spr.setTextSize(1);
  spr.setTextColor(DS_DIM, DS_CFMBG);
  const char* hint = "final tap to confirm";
  spr.setCursor((W - (int)strlen(hint) * 6) / 2, y); spr.print(hint);

  // Action footer — matches the artboard AND loop() wiring: BtnA executes the
  // reset (left, red DENYBG), BtnB cancels (right, green CXLBG).
  const int FH = 34, FY = H - FH;
  spr.fillRect(0,     FY, W / 2,     FH, DS_DENYBG);
  spr.fillRect(W / 2, FY, W - W / 2, FH, DS_CXLBG);
  spr.drawFastVLine(W / 2, FY, FH, DS_BLACK);
  spr.setTextColor(DS_REDSOFT, DS_DENYBG);
  spr.setCursor(8, FY + 12);          spr.print("A reset");
  spr.setTextColor(DS_GRNSOFT, DS_CXLBG);
  spr.setCursor(W / 2 + 8, FY + 12);  spr.print("B cancel");
}
...
static void drawReset() {
  ...
  // Otherwise the destructive list, shared look, red danger meta. On the list
  // BtnA scrolls, BtnB selects/arms a row (raising the confirm overlay). Once
  // armed the footer flips to A reset / B cancel (see drawResetConfirm).
  drawListMenu("RESET", resetItems, RESET_N, resetSel, "A scroll", "B select", resetMeta);
}
...
// DJ-mode flag + the compact landscape-clock DJ renderer. Defined later in the
// file (djActive ~line 2274) and in gr0m.cpp respectively; forward-declared here
// so drawClockLandscape can compose the COMPACT DJ into the left pane when DJ
// mode is on (instead of the normal pet), with the clock unchanged on the right.
extern bool djActive;
extern void gr0mRenderMusicEQClock(TFT_eSPI*, uint32_t, int w, int h);
  ...
  if (repaint || millis() - lastPetTick >= redrawIntervalMs) {
    ...
    if (buddyMode) {
      if (petSprReady) {
        ...
        // DJ mode: compose a COMPACT DJ (small rim-lit head + pane-width neon EQ)
        // into the 110x135 left pane instead of the normal pet, so the clock
        // reads as a music-reactive DJ + clock. The stage backdrop fills the
        // pane, so the fillSprite above just primes it. Non-DJ → normal pet.
        if (djActive) gr0mRenderMusicEQClock(&petSpr, millis(), 110, 135);
        else          buddyRenderTo(&petSpr, activeState);
        petSpr.pushSprite(0, 0);
      } else {
        M5.Lcd.fillRect(0, 0, 110, 135, p.bg);
        if (djActive) gr0mRenderMusicEQClock(&M5.Lcd, millis(), 110, 135);
        else          buddyRenderTo(&M5.Lcd, activeState);
...
static void _infoHeader(const Palette& p, int& y, const char* section, uint8_t page) {
  // Top hairline to seat the strip below the gr0m head peek, then the
  // canonical header strip (title size-2 white, counter size-1 dim).
  spr.drawFastHLine(0, y, W, DS_LINE);
  char pb[8]; snprintf(pb, sizeof(pb), "%u/%u", page + 1, INFO_PAGES);
  drawScreenHeader(section, pb, DS_DARK, y);
  y += H_HDR + 6;
  spr.setTextSize(FS_SUB);
}
...
void drawPasskey() {
  ...
  // Header strip — canonical (title size-2 white on the blue pairing band).
  // "BLUETOOTH PAIRING" can't fit at the size-2 HEADER floor (17 chars), so
  // the title is SHORTENED to "PAIRING"; the blue band carries the bluetooth
  // semantics. Same strip helper every screen uses.
  drawScreenHeader("PAIRING", nullptr, DS_PASSHDR);
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    // Telemetry rows use Font 2 (8×16) — the kv-data-row body floor, identical
    // to the CLAUDE page's lg() rows — instead of the old size-1 ln() text,
    // which read as a caption rather than primary data. Paired values are
    // combined onto one line so the full readout still fits the column.
    spr.setTextColor(p.textDim, p.bg);
    lg("bat %d.%02dV %+dmA", vBat_mV/1000, (vBat_mV%1000)/10, iBat_mA);
    if (usb) lg("usb %d.%02dV in", vBus_mV/1000, (vBus_mV%1000)/10);

    spr.setTextColor(DS_DIM, p.bg);
    spr.setTextSize(FS_SUB);
    spr.setCursor(4, y); spr.print("SYSTEM"); y += 9;
    spr.setTextColor(p.textDim, p.bg);
    uint32_t up = millis() / 1000;
    lg("up %luh%02lum  %uKB", up / 3600, (up / 60) % 60, ESP.getFreeHeap() / 1024);
    lg("br %u/4  bt %s", brightLevel, bleSuspended() ? "wifi"
                       : settings().bt ? (dataBtActive() ? "link" : "on") : "off");
    lg("temp %dC", (int)M5.Axp.GetTempInAXP192());
    if (ownerName()[0]) lg("owner %s", ownerName());
    ...
    y += 8;
    // LEVEL row — dim LABEL (size 1) + the level VALUE at the size-2 floor
    // (was size 1, which read as a caption). Right-aligned value; the label
    // sits on the size-2 baseline mid-line so they share the row.
    spr.setTextSize(FS_SUB);
    spr.setTextColor(DS_DIM, p.bg);
    spr.setCursor(4, y + 4); spr.print("LEVEL");
    spr.setTextSize(FS_BODY);
    spr.setTextColor(DS_WHITE, p.bg);
    char lvb[8]; snprintf(lvb, sizeof(lvb), "%u", stats().level);
    spr.setCursor(W - 4 - (int)strlen(lvb) * 12, y); spr.print(lvb);
    spr.setTextSize(FS_SUB);
    y += 16;
...
static void drawApproval() {
  ...
  // Tool name at the size-2 VALUE floor (was FS_HERO size 3 — shrunk per user
  // request so "Bash" reads compact and longer names fit without dominating the
  // panel). Size 1 is captions only, so size 2 is the floor; ~10 chars fit at
  // size 2, longer tool names hard-truncate below.
  uint8_t toolSize = FS_BODY;
  spr.setTextSize(toolSize);
  spr.setTextColor(p.text, p.bg);
  spr.setCursor(6, TOP + 38);
  // At the size-2 floor only ~10 glyphs fit across 135 px — hard-truncate the
  // NAME (not the font) so it never overflows, per the type-system rule.
  if (toolSize == FS_BODY && toolLen > 10) {
    char tn[11]; snprintf(tn, sizeof(tn), "%.9s>", tama.promptTool);
    spr.print(tn);
  } else {
  ...
  //   size-3 tool → 1 preview line, size-2 tool → up to 2 lines.
  int previewY = TOP + 38 + (toolSize == FS_HERO ? 26 : 18);
  spr.setTextColor(p.textDim, p.bg);
...
static void drawPetStats(const Palette& p) {
  ...
  // Starts at y=94 to clear the canonical 20-px header strip (drawPetHeader).
  int y = 94;
  ...
  // ── BATTERY METER ───────────────────────────────────────────────
  // Real AXP192 battery (per request) in place of the nap-energy bars:
  // BATTERY label + % + USB badge, then a battery shell filled to charge and
  // colored by level (green ok / amber mid / red low).
  y = 140;
  uint8_t pct = batteryPct();
  ...
  if (onUsb) {
    ...
    spr.setCursor(W - 4 - 3 * 6, y);
    spr.print("USB");
  }
  spr.setTextColor(batCol, p.bg);
  spr.setCursor(58, y);
  spr.printf("%u%%", pct);
  // Battery shell + terminal nub, filled to pct.
  {
    int by = y + 14, bw = W - 18, bh = 11;
...
static void drawPetHowTo(const Palette& p) {
  const int TOP = 70;
  spr.fillRect(0, TOP, W, H - TOP, p.bg);
  int y = TOP + 24;            // clear the canonical PET header strip (drawPet)
...
static void drawPetHeader(const Palette& p) {
  const int HY = 70;
  // Title = pet name, or "owner's pet" when an owner is set and differs.
  // The HEADER role is ALWAYS size 2 white (never the size-1 fallback the
  // old strip dropped to) — to keep long "owner's pet" titles inside the
  // 135 px strip we SHORTEN the text (drop the pet name, keep "owner's"),
  // never the font, per the type-system floor.
  char title[40];
  const char* owner = ownerName();
  if (owner[0] && strcasecmp(owner, petName()) != 0) {
    snprintf(title, sizeof(title), "%s's %s", owner, petName());
    if (strlen(title) > 10) snprintf(title, sizeof(title), "%s's", owner);
    if (strlen(title) > 10) snprintf(title, sizeof(title), "%.8s", petName());
  } else {
    snprintf(title, sizeof(title), "%s", petName());
  }
  char pb[8]; snprintf(pb, sizeof(pb), "%u/%u", petPage + 1, PET_PAGES);
  drawScreenHeader(title, pb, DS_DARK, HY);
}
...
static void drawHomeTranscript(const Palette& p, int topY) {
  // Transcript rows render in Font 2 (8×16 proportional) — smaller/slimmer than
  // size-2 (12×16) per user request, and consistent with the prompt command
  // preview. WIDTH widens to ~16 since the narrower glyphs fit more per line.
  const int SHOW = 2, LH = 16, WIDTH = 16;
...
void drawHUD() {
  ...
  if (!busy) {
    // ScreenHomeIdle — "TOKENS TODAY" caption + hero number (period tokens
    // mirror). The "TOKENS"/"TODAY" lines are the dim LABEL paired with the
    // hero value → size-1 DS_DIM, identical to the USAGE "TODAY" label and the
    // pet-stats captions (was size-2, the value floor, which read as a value).
    const int STAT_TOP = 168;
    spr.fillRect(0, STAT_TOP, W, H - STAT_TOP, p.bg);
    spr.drawFastHLine(0, STAT_TOP, W, DS_LINE);
    spr.setTextSize(FS_SUB);
    spr.setTextColor(DS_DIM, p.bg);
    spr.setCursor(6, STAT_TOP + 8);  spr.print("TOKENS");
    spr.setCursor(6, STAT_TOP + 20); spr.print("TODAY");
    char tb[12]; char unit = 0;
    fmtTokens(tb, sizeof(tb), g_dispTokens, &unit);
    spr.setTextSize(3);
    spr.setTextColor(DS_WHITE, p.bg);
    spr.setCursor(6, STAT_TOP + 36); spr.print(tb);
    if (unit) {
      int w3 = (int)strlen(tb) * 18;
      spr.setTextSize(2);
      spr.setTextColor(DS_DIM, p.bg);
      spr.setCursor(6 + w3 + 2, STAT_TOP + 43);
      spr.print(unit);
    }
    return;
  }
...
void setup() {
  ...
  // Battery saver: drop the CPU from 240 MHz to 80 MHz. The render is SPI/DMA
  // bound (pushSprite), not CPU bound, so 80 MHz still holds 60fps while cutting
  // core current substantially. 80 MHz is the documented floor that keeps the
  // WiFi radio working; lower would break the association. The APB clock stays
  // 80 MHz, so the I2S mic, ledc beeper, and Serial baud are all unaffected.
...
// DJ mode flag — a user toggle from Settings -> dj. When on, DJ mode REPLACES
// the gr0m CHARACTER with a mic-reactive music EQ visualizer and enables mic
// sampling so the EQ reacts. It does NOT take over the screen: the HUD chrome,
// all screens (home/info/pet/usage/...) and all menus render normally on top —
// DJ mode only swaps the character layer. Recomputed every frame in loop().
bool djActive = false;
// Music-EQ visualizer renderer — lives in gr0m.cpp (to reach its file-static
// draw primitives + the live mic spectrum feed), composed onto the surface we
// pass in. Renders in place of the gr0m character when djActive.
extern void gr0mRenderMusicEQ(TFT_eSPI*, uint32_t);
// (Legacy booth renderer — retired from the render path. Kept extern-declared
//  in gr0m.cpp but no longer called: DJ mode is the EQ now, no booth, no BPM.)
...
// Showcase outfit rotation (Settings ▸ cycle) — gr0m.cpp renders one finished
// look per index (0 robot / 1 DJ / 2 costume-undercover / 3 costume-blond /
// 4 skate / 5 surf / 6 hover / 7 green-digital LCD chest); the bolt is the
// chest mark on every look EXCEPT 7. gr0mOutfitCount() is how many there are.
extern void gr0mRenderOutfit(TFT_eSPI*, uint32_t, uint8_t outfit);
extern uint8_t gr0mOutfitCount();
...
static void drawAdapter() {
  ...
  spr.fillSprite(DS_BLACK);
  // Header — canonical strip on the amber adapter band: title "ADAPTER" at the
  // size-2 white HEADER role (was size-1 amber). The PROBE status tag is a
  // size-1 dim caption with its amber status dot, drawn into the right of the
  // strip in place of a page counter.
  drawScreenHeader("ADAPTER", nullptr, DS_ADPHDR);
  drawStatusDot(W - 44, H_HDR / 2, DS_AMBER, 3, true);
  spr.setTextSize(FS_SUB);
  spr.setTextColor(DS_DIM, DS_ADPHDR);
  spr.setCursor(W - 36, 7); spr.print("PROBE");
...
void loop() {
  ...
  // DJ mode: a user toggle from Settings -> dj. When on it REPLACES the gr0m
  // character with a mic-reactive music EQ visualizer and enables mic sampling
  // so the EQ reacts — it does NOT take over the screen. The HUD chrome, all
  // screens and all menus render normally; only the character layer is swapped
  // (see the render path below). Authoritative (NOT gated on evoStage).
  djActive = settings().dj;

  // Mic sampling follows DJ: while DJ is on, force sampling ON so the EQ has a
  // live spectrum to react to; when DJ is off, restore the user's explicit
  // Settings -> mic opt-in. This is sampling ONLY — the clap-to-approve ACTION
  // is gated on settings().mic (the opt-in), not on sampling, so DJ-enabled
  // sampling never arms clap approve/deny (see the prompt clap gate below).
  {
    bool wantSampling = djActive || settings().mic;
    if (wantSampling != micEnabled()) micSetEnabled(wantSampling);
  }
  ...
  //
  // Gate the ACTION on settings().mic (the user's explicit opt-in), NOT on
  // micEnabled() (whether sampling is running). DJ mode forces sampling on for
  // the EQ, but that must NOT arm clap approve/deny — only the deliberate
  // Settings -> mic opt-in does. micClapsConsume() is still drained either way
  // so a stray clap doesn't queue up and fire the moment mic is later enabled.
  micTick();
  {
    uint8_t claps = micClapsConsume();
    if (inPrompt && settings().mic) {
  ...
  if (M5.BtnA.wasReleased()) {
    if (!btnALong && !swallowBtnA) {
      if (inPrompt) {
        ...
        // Artboard ScreenConfirmDestructive: BtnA is the destructive action.
        // While armed (the confirm overlay is up) BtnA is the FINAL TAP that
        // executes the reset; otherwise BtnA scrolls the list (which disarms
        // any pending confirm). BtnB cancels — see the BtnB handler below.
        bool armed = (resetConfirmIdx == resetSel)
                  && (int32_t)(millis() - resetConfirmUntil) < 0
                  && resetSel < 2;
        if (armed) {
          beep(2400, 30);
          applyReset(resetSel);          // armed → execute
        } else {
          beep(1800, 30);
  ...
  if (M5.BtnB.wasPressed()) {
    ...
    if (inPrompt) {
      ...
      // Artboard ScreenConfirmDestructive footer: BtnB = CANCEL. When a
      // destructive row is armed (overlay up) BtnB disarms it (cancel); when
      // not yet armed, BtnB arms it / selects the row (raises the overlay).
      // The final execute tap is BtnA — see the BtnA-release handler above.
      bool armed = (resetConfirmIdx == resetSel)
                && (int32_t)(millis() - resetConfirmUntil) < 0
                && resetSel < 2;
      if (armed) {
        beep(1200, 40);
        resetConfirmIdx = 0xFF;          // armed → cancel (disarm)
      } else {
        beep(2400, 30);
        applyReset(resetSel);            // not armed → arm (or run "back")
      }
    } else if (settingsOpen) {
      beep(2400, 30);
  ...
  // DJ mode no longer takes over the screen. It only swaps the CHARACTER layer
  // for a mic-reactive music EQ (rendered below in place of buddyTick); the HUD
  // chrome, all screens and all menus still render normally on top.
  ...
  if (napping || screenOff || landscapeClock) {
    ...
    if (settings().cycle) {
      // Showcase (Settings ▸ cycle): rotate through the finished OUTFITS —
      // full-gear robot, DJ booth, both human-costume variants, and the three
      // rides — a new one every SHOWCASE_MS. Clears the buddy zone like the DJ
      // path, then dispatches to gr0m.cpp's outfit renderer; the HUD chrome /
      // screens / menus still draw over it unchanged. Evolution stages are NOT
      // part of this — only the polished designs.
      uint8_t bs = buddyScale();
      spr.fillRect(0, 0, BUDDY_CANVAS_W, (BUDDY_Y_BASE + 5 * BUDDY_CHAR_H + 12) * bs, BUDDY_BG);
      uint8_t outfit = (now / SHOWCASE_MS) % gr0mOutfitCount();
      gr0mRenderOutfit(&spr, now, outfit);
    } else if (djActive) {
      // DJ on: replace the gr0m character with the music EQ visualizer. Clear
      // the same buddy zone buddyTick would (full width, body band scaled by
      // buddyScale()), then paint the EQ. The EQ animates every frame (no tick
      // gating) so the bars stay live. drawHUD()/drawInfo()/drawPet()/menus all
      // draw over this afterward, unchanged — only the character layer changed.
      //
      // COMPOSES with the PORTRAIT clock (no longer suppressed): the full DJ
      // scene lives in y18..142 (stage glow y18..148, EQ floor+rail y142/143)
      // and drawClockPortrait() only fillRects y170+, so the DJ booth and the
      // pinned-at-the-bottom time share the screen with a clean ~28px gap
      // (y142..170) and never overlap. The LANDSCAPE clock is excluded by the
      // `landscapeClock` skip above (it draws direct-to-LCD and composes its own
      // COMPACT DJ into petSpr inside drawClockLandscape).
      uint8_t bs = buddyScale();
      spr.fillRect(0, 0, BUDDY_CANVAS_W, (BUDDY_Y_BASE + 5 * BUDDY_CHAR_H + 12) * bs, BUDDY_BG);
      gr0mRenderMusicEQ(&spr, now);
    } else {
      buddyTick(activeState);
    }
  } else if (characterLoaded()) {
  ...
  if (landscapeClock) {
    ...
    // Blit the composed sprite to the screen, upright.
    spr.pushSprite(0, 0);
  }
  ...
  // Adaptive frame rate (aggressive saver). Run full 60fps only while something
  // is actually animating or the user is interacting; otherwise idle at ~10fps
  // so the core spends most of each idle second parked in delay() instead of
  // re-running a 5fps-gated redraw 60× a second. buddyTick already caps the idle
  // redraw at 5fps, so the slow loop looks identical — it just stops spinning.
  // Button/shake/prompt events still latch via M5.update()/dataPoll each tick,
  // so worst-case input latency is ~100ms (a buddy can tolerate that).
  bool animActive = inPrompt || djActive || menuOpen || settingsOpen || connOpen
                 || usageOpen || resetOpen || landscapeClock || settings().cycle
                 || (int32_t)(now - oneShotUntil) < 0
                 || (int32_t)(now - wakeTransitionUntil) < 0;
  delay(screenOff ? 100 : (animActive ? 16 : 100));
}
