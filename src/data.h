...
static void _applyJson(const char* line, TamaState* out) {
  // Echo every received line to USB serial — lets you confirm what the
  // desktop is actually sending (e.g. whether `tokens` / `tokens_today`
  // fields are populated). Cheap; only consumed if a serial monitor is
  // attached.
  ...
  if (evt && strcmp(evt, "turn") == 0) {
    ...
    if (role && strcmp(role, "assistant") == 0) {
      ...
      if (!content.isNull()) {
        size_t off = 0;
        out->responsePreview[0] = 0;
        for (JsonVariant c : content) {
          const char* type = c["type"];
          if (type && strcmp(type, "text") == 0) {
            const char* text = c["text"];
            if (!text) continue;
            size_t avail = sizeof(out->responsePreview) - off - 1;
            if (avail == 0) break;
            size_t len = strlen(text);
            if (len > avail) len = avail;
            // Skip newlines for the single-line banner
            for (size_t i = 0; i < len; i++) {
              char ch = text[i];
              if (ch == '\n' || ch == '\r' || ch == '\t') ch = ' ';
              out->responsePreview[off++] = ch;
            }
            out->responsePreview[off] = 0;
          }
        }
        out->responseRcvdMs = millis();
      }
    }
    _lastLiveMs = millis();
    return;
